...
CLASSIFIED: dict[str, tuple[str, str]] = {
    ...
    '/mcp': (
        ...
        'ANSWERED 2026-08-07, and the answer is no: an anonymous caller is NOT '
        ...
        'tools/list returns 200 and the full five-tool inventory without even an '
        'initialize, and tools/call returns 200 having EXECUTED the tool body '
        'with user_id=None. Nothing rejects it: /mcp is outside /api so '
        ...
        'Upstream DOES authenticate this endpoint, which is the part worth not '
        'misreading: enterprise/server/middleware.py returns True for '
        'path.startswith("/mcp") in its should-authenticate predicate, and '
        'SaasUserAuth.get_mcp_api_key mints the key the sandbox presents as '
        'X-Session-API-Key. That middleware lives in enterprise/ and is not part '
        'of this deployment. NimbusUserAuth.get_mcp_api_key returns None, so the '
        'LEGITIMATE sandbox also calls /mcp/mcp with no credential and also '
        'resolves to user_id=None — which is why the gap is invisible in normal '
        'use, and why closing it means giving the sandbox a credential first '
        'rather than just adding a check. Deliberately NOT patched from this '
        'file; a fix that denies anonymous /mcp before the sandbox has something '
        'to send would break create_pr for real users.',
    ),
    '/openapi.json': (
        PUBLIC,
