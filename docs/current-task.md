...
1. **P15 — refusal failover.** Best effort-to-value item in the whole
   ...
   Size S–M.
2. **P11 — queue control.** Queued messages no longer vanish (they render as the
...
3. **P13 — live preview tab.** Fully designed against this codebase; see the
   P13 task and roadmap §7. Phase 1 is a ~150-line sibling of
   `sandbox/agent_proxy_router.py`, which already solves HTTP+WS proxying into
   the sandbox.
...
The **diff rendering** and the **composer model chip**. The event fixture
(`src/mocks/v1-events-ws.ts`) sends its transcript once on connect, so a
reconnect empties the view before a row can be expanded. Making the fixture
replay on reconnect would close this.
